...
The following XSD files are shipped with the model:
...
They validate the XML structures generated by the corresponding CLI outputs.
Input configuration remains YAML-based in model version 1; the XSDs describe the
XML output contracts, not an alternative XML configuration-input syntax.
